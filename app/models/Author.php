...
class Author
{
    ...
    {
        $this->db->query(
            "SELECT * FROM authors ORDER BY sinta_score_overall DESC LIMIT :limit"
        );
        $this->db->bind(":limit", $limit);
        return $this->db->resultSet();
    }
    ...
    ';
        ...
        if ($year) {
            $sql .=
                " JOIN articles ar ON aa.id_article = ar.id_article WHERE ar.published LIKE :year ";
        } else {
    ...
    public function getTopAuthorsByRangeCount(
        $limit = 5,
        $startYear,
        $endYear,
        $faculty = null
    ...
    public function getTopAuthorsByPublicationCount(
        $limit = 10,
        $faculty = null,
        $year = null
    ) {
    ...
    public function getTopAuthorsByFaculty(
        $faculty = null,
        $year = null,
        $limit = 5
    ) {
    ...
    {
        $this->db->query(
            'SELECT DISTINCT faculty FROM authors WHERE faculty IS NOT NULL AND faculty != "" ORDER BY faculty ASC'
        );
        return $this->db->resultSet();
    }

    // Update only the SINTA-sourced bibliometric fields for an existing author.
    // Fields not captured by the SINTA scraper (nidn, faculty) are left untouched.
    // degree & major are synced after being normalized in ScrapingController.
    public function updateFromSinta($idSinta, array $data)
    {
        $this->db->query('
      UPDATE authors SET
        fullname                  = :fullname,
        ...
        s_article_scopus          = :s_article_scopus,
        s_citation_scopus         = :s_citation_scopus,
        s_cited_document_scopus   = :s_cited_document_scopus,
        s_hindex_scopus           = :s_hindex_scopus,
        s_i10_index_scopus        = :s_i10_index_scopus,
        s_gindex_scopus           = :s_gindex_scopus,
        s_article_gscholar        = :s_article_gscholar,
        s_citation_gscholar       = :s_citation_gscholar,
        s_cited_document_gscholar = :s_cited_document_gscholar,
        s_hindex_gscholar         = :s_hindex_gscholar,
        s_i10_index_gscholar      = :s_i10_index_gscholar,
        s_gindex_gscholar         = :s_gindex_gscholar
      WHERE id_sinta = :id_sinta
    ');
        ...
        $this->db->bind(
            ":sinta_score_overall",
            $data["sinta_score_overall"] ?? null
        );
        $this->db->bind(":sinta_score_3yr", $data["sinta_score_3yr"] ?? null);
        $this->db->bind(":affil_score", $data["affil_score"] ?? null);
        $this->db->bind(":affil_score_3yr", $data["affil_score_3yr"] ?? null);
        $this->db->bind(":s_article_scopus", $data["s_article_scopus"] ?? null);
        $this->db->bind(
            ":s_citation_scopus",
            $data["s_citation_scopus"] ?? null
        );
        $this->db->bind(
            ":s_cited_document_scopus",
            $data["s_cited_document_scopus"] ?? null
        );
        $this->db->bind(":s_hindex_scopus", $data["s_hindex_scopus"] ?? null);
        $this->db->bind(
            ":s_i10_index_scopus",
            $data["s_i10_index_scopus"] ?? null
        );
        $this->db->bind(":s_gindex_scopus", $data["s_gindex_scopus"] ?? null);
        $this->db->bind(
            ":s_article_gscholar",
            $data["s_article_gscholar"] ?? null
        );
        $this->db->bind(
            ":s_citation_gscholar",
            $data["s_citation_gscholar"] ?? null
        );
        $this->db->bind(
            ":s_cited_document_gscholar",
            $data["s_cited_document_gscholar"] ?? null
        );
        $this->db->bind(
            ":s_hindex_gscholar",
            $data["s_hindex_gscholar"] ?? null
        );
        $this->db->bind(
            ":s_i10_index_gscholar",
            $data["s_i10_index_gscholar"] ?? null
        );
        $this->db->bind(
            ":s_gindex_gscholar",
            $data["s_gindex_gscholar"] ?? null
        );

        return $this->db->execute();
    }

    // Insert a brand-new author discovered by the SINTA scraper.
    // Fields unavailable from SINTA (nidn, faculty) are left NULL and
    // can be populated later from another data source.
    public function insertFromSinta(array $data)
    {
        $this->db->query('
      INSERT INTO authors (
        id_sinta,
    ...
    ');
        ...
        $this->db->bind(":id_sinta", intval($data["id_sinta"]));
        $this->db->bind(":fullname", $data["fullname"] ?? null);
        $this->db->bind(":degree", $data["degree"] ?? null);
        $this->db->bind(":major", $data["major"] ?? null);
        $this->db->bind(":faculty", $data["faculty"] ?? null);
